    ...
    case WM_PAINT:
        {
			...
			rt.bottom = 150;
			TCHAR buf[1024] = { 0 };
			_tcscpy_s(buf, L"你选择的奇遇是");
			//_stprintf_s(buf, L"你选择的奇遇是");
			if (bLove[0])
			{
				_tcscat_s(buf, L" 阴阳两界");
			}
			if (bLove[1])
			{
				_tcscat_s(buf, L" 三山四海");
			}
			if (bLove[2])
			{
				_tcscat_s(buf, L" 三尺青锋");
			}
