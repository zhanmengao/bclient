...
#define WINDOW_TITLE	L"透明动画"	      //为窗口标题定义的宏
...
bool Game_init(const HWND& hwnd)
{
	...
	g_iX = 0;
	g_iY = 350;
	//贴一个空位图到g_mdc，因为我们要在g_mdc上进行透明处理
...
void Game_Paint(const HWND& hwnd)
{
	...
	TransparentBlt(g_mdc, g_iX, g_iY, 60, 108, g_bufdc, g_iNum * 60, 0, 60, 108, RGB(255, 0, 0));   //从
	//将处理好的贴到hdc
