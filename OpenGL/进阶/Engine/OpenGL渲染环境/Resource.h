...
public:
	...
	static GLuint GetProgramTexture(int halfSize);//获取从res中读到的文件
	//static void RemoveProgramTexture(GLuint texture);

	static GLuint GetTextureCube(const char* front,const char* back,const char* top,const char* bottom,const char* left,const char* right);//获取从res中读到的文件
...
private:
	...
	//生成一个程序纹理
	static GLuint CreateProcedureTexture(const int& size, ALPHA_TYPE type = ALPHA_TYPE::ALPHA_GAUSSIAN);
...
private:
	...
	//程序纹理管理
	static std::map<int, Texture> m_mProgramTexture;
	//GPU程序管理
