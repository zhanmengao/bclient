...
void ResourceManager::RemovePic(GLuint texture)
{
	...
	for (auto it = m_mTexture.begin(); it != m_mTexture.end();it++)
	{
		...
		{
			it->second.CiteCount--;
			cout << "卸载图片" << it->first << ",剩余引用数" << it->second.CiteCount << endl;
	...
	for (auto it = m_mProgramTexture.begin(); it != m_mProgramTexture.end(); it++)
	{
		...
		{
			it->second.CiteCount--;
			cout << "卸载程序纹理" << it->first << ",剩余引用数" << it->second.CiteCount << endl;
			if (it->second.CiteCount <= 0)
			{
				glDeleteTextures(1, &(it->second.texture));
...
void ResourceManager::RemoveTextureCube(GLuint texture)
{
	for (auto it = m_mTextureCube.begin(); it != m_mTextureCube.end(); it++)
	{
		...
		{
			it->second.CiteCount--;
			cout << "销毁CubeMap:" << it->first<<"剩余引用次数"<<it->second.CiteCount << endl;
...
void ResourceManager::RemoveProgram(GLuint program)
{
	for (auto it = m_mProgram.begin(); it != m_mProgram.end();)
	{
		...
		{
			it->second.CiteCount--;
			cout << "卸载程序" << it->first << ",剩余引用数" << it->second.CiteCount << endl;
			if (it->second.CiteCount <= 0)
			{
...
void ResourceManager::RemoveModel(string path)
{
	...
	{
		it->second.CiteCount--;
		cout << "卸载模型" << it->first << ",剩余引用数" << it->second.CiteCount<<endl;
		if (it->second.CiteCount <= 0)
		{
			m_mModel.erase(it);
		}
...
GLuint ResourceManager::CreateTexture2D(const char* fileName, bool isRepeat)
{
	...
	*/
	//设置参数
	//举例：当128*128的纹理映射到256*256的物体上时（纹理扩大），使用线性过滤
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	//举例：当128*128的纹理映射到64*64的物体上时（纹理缩小），使用线性过滤
...
GLuint ResourceManager::CreateCubeMap(const char* front, const char* back, const char* top, const char* bottom, const char* left, const char* right)
{
	...
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);//举例：当128*128的纹理映射到256*256的物体上时（纹理扩大），使用线性过滤
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR);//举例：当128*128的纹理映射到64*64的物体上时（纹理缩小），使用线性过滤
